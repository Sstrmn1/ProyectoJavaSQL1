...
public class ArticuloDAO implements CrudSimpleInterface<Articulo> {
    ...
    public List<Articulo> listar(String texto) {
        ...
        try {
            ps = CON.conectar().prepareStatement("SELECT a.id_articulo, a.id_ffarmaceutica, a.codigo, a.descripcion, a.concentracion,\n"
            ...
            while (rs.next()) {
                registros.add(new Articulo(rs.getInt("id_articulo"),
                        ...
                        rs.getString("concentracion"), rs.getString("codigo"),
    ...
    public boolean insertar(Articulo obj) {
        respuesta = false;
        try {
            ps = CON.conectar().prepareStatement("INSERT INTO articulo (id_ffarmaceutica, descripcion, concentracion, codigo, activo) VALUES \n"
                    + "(?,?,?,?,?);");
            ps.setInt(1, obj.getIdFfarmaceutica());
            ps.setString(2, obj.getDescripcion());
            ps.setString(3, obj.getConcentracion());
            ps.setString(4, obj.getCodigo());
            ps.setInt(5, obj.isActivo() ? 1 : 0);
